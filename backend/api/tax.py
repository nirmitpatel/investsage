...
def _fetch_opportunities_sync(user_id: str):
    ...
    return opportunities, summary, has_lots, portfolio
...
async def get_tax_opportunities(user_id: str = Depends(get_current_user)):
    ...
    opportunities, summary, has_lots, portfolio = await asyncio.to_thread(_fetch_opportunities_sync, user_id)
    return {
        "summary": summary,
...
async def explain_opportunity(
    ...
    opportunities, _, _has, _ = await asyncio.to_thread(_fetch_opportunities_sync, user_id)
